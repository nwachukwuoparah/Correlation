...
function tableFor(event, journal) {
  let table = [0, 0, 0, 0];
  //   loop through journal.length
  for (let i = 0; i < journal.length; i++) {
    let entry = journal[i],
      index = 0;
    //   condition to check if entry.events includes pizza
    if (entry.events.includes(event)) index += 1;
    //   condition to check if entry entry.squirrel
    if (entry.squirrel) index += 2;

    /*for any of the two conditions above
that is been satisfied +1 is been added to the defult index*/
    table[index] += 1;
  }
  return table;
}
...
function journalEvents(journal) {
  // events
  let events = [];
  // itterate through the journal
  for (let entry of journal) {
    // itterate through entry.event
    for (let event of entry.events) {
      //    going through adding the element that are not in events
...
// TEST  BEEM COUNT
const count = (test, Run) => {
  if (test) Run();
};
const par = (string, charater) => {
  let result = 0;
  for (i = 0; i <= string.length; i++) {
    count(string[i] === charater, () => {
      result += 1;
    });
  }
  return result;
};
console.log(par(";klfdnskHHHHhhhhhhoaj;kcklndsknv;ks", "h"));

const countB = (string) => {
  return par(string, charater);
};
console.log(par("BB", "B"));
...
console.log(SCRIPTS.filter((s) => !s.living));
console.log(SCRIPTS.filter((s) => s.direction == "ltr")); // LTR
// ltr living
let run1 = SCRIPTS.filter((s) => s.living);
console.log(run1.filter((s) => s.direction == "ltr"));
// // ltr !living
let run2 = SCRIPTS.filter((s) => !s.living);
console.log(run2.filter((s) => s.direction == "ltr"));

console.log(SCRIPTS.filter((s) => s.direction == "rtl")); // RTL
// rtl living
let run3 = SCRIPTS.filter((s) => s.living);
console.log(run3.filter((s) => s.direction == "rtl"));
// rtl !living
let run4 = SCRIPTS.filter((s) => !s.living);
console.log(run4.filter((s) => s.direction == "rtl"));

console.log(SCRIPTS.filter((s) => s.direction == "ttb")); // TTB
// ttb living
let run5 = SCRIPTS.filter((s) => s.living);
console.log(run5.filter((s) => s.direction == "ttb"));
// ttb !living
let run6 = SCRIPTS.filter((s) => !s.living);
console.log(run6.filter((s) => s.direction == "ttb"));
